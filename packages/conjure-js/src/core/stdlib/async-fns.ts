...
export const asyncFunctions: Record<string, CljValue> = {
  ...
  then: v.nativeFnCtx('then', (ctx: EvaluationContext, callEnv: Env, val: CljValue, f: CljValue) => {
    if (!isCallable(f)) {
  ...
  'catch*': v.nativeFnCtx('catch*', (ctx: EvaluationContext, callEnv: Env, val: CljValue, f: CljValue) => {
    if (!isCallable(f)) {
      throw new EvaluationError(`${printString(f)} is not a callable value`, { fn: f, args: [] })
    }
    ...
    const promise = val.promise.catch(err => {
      ...
      if (err instanceof CljThrownSignal) {
        ...
        errVal = {
          kind: 'map',
          entries: [
            [{ kind: 'keyword', name: ':type' }, { kind: 'keyword', name: ':error/js' }],
  ...
  // (pending? x) → boolean
  'pending?': v.nativeFn('pending?', (val: CljValue) => {
    return cljBoolean(val.kind === 'pending')
