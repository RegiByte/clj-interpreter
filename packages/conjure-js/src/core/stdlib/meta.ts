...
export const metaFunctions: Record<string, CljValue> = {
  meta: v.nativeFn('meta', function metaImpl(val: CljValue) {
    if (val === undefined) {
  ...
  'with-meta': v.nativeFn('with-meta', function withMetaImpl(val: CljValue, m: CljValue) {
    if (val === undefined) {
    ...
    return { ...val, meta }
  }).doc('Returns a new value with the metadata map m applied to val.', [['val', 'm']]),
