...
function printString(value, _depth = 0) {
  ...
  if (printLevel !== null && _depth >= printLevel) {
    if (value.kind === "list" || value.kind === "vector" || value.kind === "map" || value.kind === "set" || value.kind === "cons" || value.kind === "lazy-seq")
      return "#";
  }
...
`:
    ...
    case valueKeywords.jsValue: {
      ...
      if (raw === null) {
        ...
        typeName = raw.constructor?.name ?? "Object";
      }
      return `#<js ${typeName}>`;
    }
    default:
...
var toSeq = (collection) => {
  ...
  if (isMap(collection)) {
    return collection.entries.map(([k, v2]) => cljVector([k, v2]));
  }
  if (isSet(collection)) {
...
function destructureVector(pattern, value, ctx, env) {
  const pairs = [];
  const elems = [...pattern];
  const asIdx = elems.findIndex((e) => is.keyword(e) && e.kind === "keyword" && e.name === ":as");
  if (asIdx !== -1) {
...
function destructureMap(pattern, value, ctx, env) {
  ...
  for (const [k, v2] of pattern.entries) {
    if (is.keyword(k) && k.name === ":or")
      continue;
    if (is.keyword(k) && k.name === ":as")
      continue;
    if (is.keyword(k) && k.name === ":keys") {
      if (!is.vector(v2)) {
        throw new EvaluationError(":keys must be followed by a vector of symbols", { pattern });
      }
      for (const sym of v2.value) {
        if (!is.symbol(sym)) {
    ...
    const entry = findMapEntry(targetMap, v2);
    const present = mapContainsKey(targetMap, v2);
    let boundVal;
    if (present) {
      boundVal = entry;
...
function parseParamVector(args, env) {
  ...
  if (ampIdx === -1) {
    ...
    const ampsCount = args.value.filter((a) => is.symbol(a) && a.name === "&").length;
    if (ampsCount > 1) {
...
async function evaluateSpecialFormAsync(name, list, env, asyncCtx) {
  switch (name) {
    ...
    case "if": {
      ...
      const isTruthy = condition.kind !== "nil" && !(condition.kind === "boolean" && !condition.value);
      if (isTruthy) {
        return evaluateFormAsync(list.value[2], env, asyncCtx);
      }
      return list.value[3] !== undefined ? evaluateFormAsync(list.value[3], env, asyncCtx) : cljNil();
    }
    case "let":
    case "let*":
...
async function evaluateLetAsync(list, env, asyncCtx) {
  ...
  if (bindings.value.length % 2 !== 0) {
    throw new EvaluationError("let bindings must have an even number of forms", { list, env });
  }
  let currentEnv = env;
...
async function evaluateLoopAsync(list, env, asyncCtx) {
  ...
  if (loopBindings.value.length % 2 !== 0) {
    throw new EvaluationError("loop bindings must have an even number of forms", { list, env });
  }
...
async function evaluateTryAsync(list, env, asyncCtx) {
  ...
  for (let i = 0;i < forms.length; i++) {
    const form = forms[i];
    if (form.kind === "list" && form.value.length > 0 && form.value[0].kind === "symbol") {
      const head = form.value[0].name;
      if (head === "catch") {
        catchClauses.push({
          discriminator: form.value[1],
          binding: form.value[2].name,
...
function jsToClj(raw) {
  if (raw === null)
    return cljNil();
  if (raw === undefined)
    return cljJsValue(undefined);
  if (typeof raw === "number")
    return cljNumber(raw);
  if (typeof raw === "string")
    return cljString(raw);
  if (typeof raw === "boolean")
    return cljBoolean(raw);
...
function cljToJs(val, ctx, callEnv) {
  switch (val.kind) {
    case "js-value":
      return val.value;
    case "number":
    ...
    case "list":
    case "vector":
      return val.value.map((v2) => cljToJs(v2, ctx, callEnv));
    case "map": {
      const obj = {};
...
function extractRawTarget(target) {
  switch (target.kind) {
    case "js-value":
      return target.value;
    case "string":
...
function evaluateDot(list, env, ctx) {
  if (list.value.length < 3) {
    throw new EvaluationError(". requires at least 2 arguments: (. obj prop)", { list });
  }
  ...
  if (rawTarget === null || rawTarget === undefined) {
    ...
    throw new EvaluationError(`cannot use . on ${label} js value — check for nil/undefined before accessing properties`, { target });
  }
  const propForm = list.value[2];
  if (!is.symbol(propForm)) {
    throw new EvaluationError(`. expects a symbol for property name, got: ${propForm.kind}`, { propForm });
  }
  ...
  if (list.value.length === 3) {
    ...
    if (typeof rawProp === "function") {
      return cljJsValue(rawProp.bind(rawObj));
    }
    return jsToClj(rawProp);
  }
  const method = rawObj[propName];
  if (typeof method !== "function") {
    throw new EvaluationError(`method '${propName}' is not callable on ${String(rawObj)}`, { propName, rawObj });
  }
...
function evaluateNew(list, env, ctx) {
  ...
  if (!is.jsValue(cls) || typeof cls.value !== "function") {
    throw new EvaluationError(`js/new: expected js-value constructor, got ${cls.kind}`, { cls });
  }
...
function evaluateQuasiquote(form, env, autoGensyms = new Map, ctx) {
  switch (form.kind) {
    ...
    case valueKeywords.list: {
      ...
      if (isAList && form.value.length === 2 && is.symbol(form.value[0]) && form.value[0].name === "unquote") {
        return ctx.evaluate(form.value[1], env);
      }
      const elements = [];
      for (const elem of form.value) {
        if (is.list(elem) && elem.value.length === 2 && is.symbol(elem.value[0]) && elem.value[0].name === "unquote-splicing") {
          ...
          } else if (is.nil(toSplice)) {} else {
            throw new EvaluationError("Unquote-splicing must evaluate to a seqable", { elem, env });
          }
          continue;
        }
...
function evaluateLet(list, env, ctx) {
  ...
  for (let i = 0;i < bindings.value.length; i += 2) {
    ...
    localEnv = extend(pairs.map(([n]) => n), pairs.map(([, v2]) => v2), localEnv);
  }
...
function evaluateLetfn(list, env, ctx) {
  ...
  for (const spec of fnSpecs.value) {
    const name = spec.value[0];
    const fn = sharedEnv.bindings.get(name.name);
    fn.env = sharedEnv;
  }
...
function evaluateLoop(list, env, ctx) {
  ...
  for (let i = 0;i < loopBindings.value.length; i += 2) {
    ...
    initEnv = extend(pairs.map(([n]) => n), pairs.map(([, v2]) => v2), initEnv);
  }
  ...
  while (true) {
    ...
    for (let i = 0;i < patterns.length; i++) {
      ...
      loopEnv = extend(pairs.map(([n]) => n), pairs.map(([, v2]) => v2), loopEnv);
    }
    try {
...
function evaluateBinding(list, env, ctx) {
  ...
  for (let i = 0;i < bindings.value.length; i += 2) {
    ...
    if (!v2.dynamic) {
      throw new EvaluationError(`Cannot use binding with non-dynamic var ${v2.ns}/${v2.name}. Mark it dynamic with (def ^:dynamic ${sym.name} ...)`, { sym });
    }
    v2.bindingStack ??= [];
...
function evaluateSet(list, env, ctx) {
  ...
  if (!v2.dynamic) {
    throw new EvaluationError(`Cannot set! non-dynamic var ${v2.ns}/${v2.name}. Mark it with ^:dynamic.`, { symForm, env });
  }
  if (!v2.bindingStack || v2.bindingStack.length === 0) {
    throw new EvaluationError(`Cannot set! ${v2.ns}/${v2.name} — no active binding. Use set! only inside a (binding [...] ...) form.`, { symForm, env });
  }
  const newVal = ctx.evaluate(list.value[2], env);
  v2.bindingStack[v2.bindingStack.length - 1] = newVal;
  return newVal;
}
function evaluateDelay(list, env, ctx) {
...
var isCallable = (value) => isAFunction(value) || isKeyword(value) || isMap(value) || isJsValue(value) && typeof value.value === "function";
...
function macroExpandAllWithContext(form, env, ctx) {
  ...
  if (is.map(form)) {
    ...
    return expanded2.every(([k, v2], i) => k === form.entries[i][0] && v2 === form.entries[i][1]) ? form : cljMap(expanded2);
  }
  if (!is.list(form))
    return form;
  if (form.value.length === 0)
...
function evaluateList(list, env, ctx) {
  ...
  if (is.multiMethod(evaledFirst)) {
    const args2 = list.value.slice(1).map((v2) => ctx.evaluate(v2, env));
...
function cljToJs2(value, applier) {
  switch (value.kind) {
    ...
    case "list":
    case "vector":
      return value.value.map((item) => cljToJs2(item, applier));
    case "map": {
      const obj = {};
      for (const [k, val] of value.entries) {
        ...
        const jsKey = String(cljToJs2(k, applier));
        obj[jsKey] = cljToJs2(val, applier);
      }
      return obj;
    }
    case "function":
    case "native-function": {
      const fn = value;
      return (...jsArgs) => {
        const cljArgs = jsArgs.map((a) => jsToClj2(a));
        const result = applier.applyFunction(fn, cljArgs);
        return cljToJs2(result, applier);
      };
    }
    case "macro":
  ...
  switch (typeof value) {
    ...
    case "function": {
      ...
      return v.nativeFn("js-fn", (...cljArgs) => {
        const jsArgs = cljArgs.map((a) => cljToJs2(a, _throwingApplier));
        const result = jsFn(...jsArgs);
        return jsToClj2(result, opts);
      });
    }
    case "object": {
      if (Array.isArray(value)) {
        return v.vector(value.map((item) => jsToClj2(item, opts)));
      }
...
var utilFunctions = {
  ...
  "pr-str": v.nativeFn("pr-str", function prStrImpl(...args) {
  ...
  "pretty-print-str": v.nativeFn("pretty-print-str", function prettyPrintStrImpl(...args) {
    if (args.length === 0)
      return v.string("");
    const form = args[0];
  ...
  "prn-str": v.nativeFn("prn-str", function prnStrImpl(...args) {
    return v.string(args.map(printString).join(" ") + `
`);
  }).doc("pr-str to a string, followed by a newline.", [["&", "args"]]),
  "print-str": v.nativeFn("print-str", function printStrImpl(...args) {
    ...
    declareNs: [
      {
        ...
        vars(_ctx) {
          ...
          map.set("println", {
            value: v.nativeFnCtx("println", (ctx, callEnv, ...args) => {
...
`);
          ...
          map.set("print", {
            value: v.nativeFnCtx("print", (ctx, callEnv, ...args) => {
              withPrintContext(readPrintCtx(callEnv), () => {
                emitToOut(ctx, callEnv, args.map(valueToString).join(" "));
              });
              return v.nil();
...
`);
          ...
          map.set("pr", {
            value: v.nativeFnCtx("pr", (ctx, callEnv, ...args) => {
              withPrintContext(readPrintCtx(callEnv), () => {
          ...
          map.set("prn", {
            value: v.nativeFnCtx("prn", (ctx, callEnv, ...args) => {
              withPrintContext(readPrintCtx(callEnv), () => {
                emitToOut(ctx, callEnv, args.map((v2) => printString(v2)).join(" ") + `
`);
              });
          ...
          map.set("pprint", {
            value: v.nativeFnCtx("pprint", (ctx, callEnv, form, widthArg) => {
              if (form === undefined)
                return v.nil();
...
`);
          ...
          map.set("warn", {
            value: v.nativeFnCtx("warn", (ctx, callEnv, ...args) => {
              withPrintContext(readPrintCtx(callEnv), () => {
...
`);
          ...
          map.set("clj->js", {
            value: v.nativeFnCtx("clj->js", (ctx, callEnv, val) => {
              if (is.jsValue(val))
                return val;
              const applier = {
                applyFunction: (fn, args) => ctx.applyCallable(fn, args, callEnv)
              };
              return v.jsValue(cljToJs2(val, applier));
            })
          });
          map.set("js->clj", {
            value: v.nativeFn("js->clj", (val, opts) => {
              if (val.kind === "nil")
                return val;
              if (!is.jsValue(val)) {
                throw new EvaluationError(`js->clj expects a js-value, got ${val.kind}`, { val });
              }
              ...
              return jsToClj2(val.value, { keywordizeKeys });
            })
          });
          return map;
        }
...
// src/core/stdlib/js-namespace.ts
...
function extractJsTarget(val, fnName) {
  switch (val.kind) {
    ...
    case "boolean":
      return val.value;
    case "nil":
      throw new EvaluationError(`${fnName}: cannot access properties on nil`, { val });
    default:
    ...
    declareNs: [
      {
        ...
        vars(_ctx) {
          const map = new Map;
          map.set("get", {
            value: v.nativeFn("js/get", (obj, key, ...rest) => {
          ...
          map.set("set!", {
            value: v.nativeFnCtx("js/set!", (ctx, callEnv, obj, key, val) => {
          ...
          map.set("call", {
            value: v.nativeFnCtx("js/call", (ctx, callEnv, fn, ...args) => {
              ...
              const jsArgs = args.map((a) => cljToJs(a, ctx, callEnv));
              return jsToClj(rawFn(...jsArgs));
            })
          });
          map.set("typeof", {
            value: v.nativeFn("js/typeof", (x) => {
              switch (x.kind) {
                case "nil":
          ...
          map.set("instanceof?", {
            value: v.nativeFn("js/instanceof?", (obj, cls) => {
          ...
          map.set("null?", {
            value: v.nativeFn("js/null?", (x) => {
              return v.boolean(x.kind === "nil");
            })
          });
          map.set("undefined?", {
            value: v.nativeFn("js/undefined?", (x) => {
              return v.boolean(x.kind === "js-value" && x.value === undefined);
            })
          });
          map.set("some?", {
            value: v.nativeFn("js/some?", (x) => {
              if (x.kind === "nil")
                return v.boolean(false);
          ...
          map.set("get-in", {
            value: v.nativeFn("js/get-in", (obj, path, ...rest) => {
              if (path.kind !== "vector") {
          ...
          map.set("prop", {
            value: v.nativeFn("js/prop", (key, ...rest) => {
          ...
          map.set("method", {
            value: v.nativeFn("js/method", (key, ...partialArgs) => {
              return v.nativeFnCtx("js/method-caller", (ctx, callEnv, obj, ...callArgs) => {
                ...
                return jsToClj(method.apply(rawObj, allArgs));
              });
            })
          });
          map.set("merge", {
            value: v.nativeFnCtx("js/merge", (ctx, callEnv, ...args) => {
              const result = Object.assign({}, ...args.map((a) => cljToJs(a, ctx, callEnv)));
              return v.jsValue(result);
            })
          });
          map.set("seq", {
            value: v.nativeFn("js/seq", (arr) => {
              ...
              return v.vector(arr.value.map(jsToClj));
            })
          });
          map.set("array", {
            value: v.nativeFnCtx("js/array", (ctx, callEnv, ...args) => {
              return v.jsValue(args.map((a) => cljToJs(a, ctx, callEnv)));
            })
          });
          map.set("obj", {
            value: v.nativeFnCtx("js/obj", (ctx, callEnv, ...args) => {
              if (args.length % 2 !== 0) {
                throw new EvaluationError("js/obj: requires even number of arguments", { count: args.length });
              }
              const result = {};
              for (let i = 0;i < args.length; i += 2) {
...
function findNsForm(forms) {
  const nsForm = forms.find((f) => isList(f) && f.value.length > 0 && isSymbol(f.value[0]) && f.value[0].name === "ns");
...
function processRequireSpec(spec, currentEnv, registry, resolveNs) {
  if (!isVector(spec)) {
...
function buildRuntime(registry, coreEnv, options) {
  ...
  const runtime = {
    ...
    processRequireSpec(spec, fromEnv, ctx) {
      processRequireSpec(spec, fromEnv, registry, (nsName) => resolveNamespace(nsName, ctx));
    },
    processNsRequires(forms, fromEnv, ctx) {
      const requireClauses = extractRequireClauses(forms);
      for (const specs of requireClauses) {
        for (const spec of specs) {
          if (isVector(spec) && spec.value.length > 0 && isString(spec.value[0])) {
        ...
        for (const spec of specs) {
          if (isVector(spec) && spec.value.length > 0 && isString(spec.value[0])) {
            ...
            for (let i = 1;i < elements.length; i++) {
              if (isKeyword(elements[i]) && elements[i].name === ":as") {
                i++;
...
function buildSessionFacade(runtime, initialNs, options) {
  ...
  const session = {
    ...
    cljToJs(value) {
      return cljToJs2(value, { applyFunction: (fn, args) => ctx.applyCallable(fn, args, makeEnv()) });
    },
    evaluateForms(forms) {
      try {
        const env = runtime.getNamespaceEnv(currentNs);
...
// src/vite-plugin-clj/static-analysis.ts
...
function parseDefn(form, isPrivate, isMacro2) {
  ...
  if (bodyForms.length === 0) {
    return { name: nameSym.name, kind: "fn", arities: [], isPrivate, isMacro: isMacro2 };
  }
  ...
  return {
    ...
    configResolved(config) {
      ...
      if (options?.entrypoint) {
        ...
        try {
          ...
          console.warn(`[vite-plugin-clj] entrypoint not found: ${options.entrypoint} — falling back to auto-generated session`);
        }
      }
      scanStringRequires();
      eagerlyGenerateDts();
    ...
    load(id) {
      if (id === RESOLVED_VIRTUAL_SESSION_ID) {
        ...
        if (entrypointPath) {
          lines.push(`export function getSession() {`, `  if (!_session) {`, `    const _outputCapture = (text) => { _outputLines.push(text); };`, `    _session = __conjureFactory(__importMap, _outputCapture);`, `  }`, `  return _session;`, `}`);
        } else {
