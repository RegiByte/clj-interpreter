...
function executeInstruction(state: VmState): void {
  ...
  switch (instruction) {
    ...
    case Op.DefMacro: {
      ...
      if (!is.symbol(name)) {
        throw new EvaluationError(
          'defmacro expects a symbol constant',
          {
            instruction,
            constantIndex,
            value: name,
      ...
      if (fn === undefined) {
        throw new EvaluationError(
          'VM stack underflow on DefMacro',
          { instruction, constantIndex, ip: frame.ip, stack, chunk },
          getPos(name) ?? instructionPos
        )
      }
      if (!is.function(fn)) {
        throw new EvaluationError(
          'defmacro expects a function value',
          {
            instruction,
            constantIndex,
            value: fn,
      ...
      const macro = v.multiArityMacro(fn.arities, fn.env)
      macro.name = name.name
      if (fn.meta) macro.meta = fn.meta
      stack.push(defineMacro({ name, macro, env, ctx }))
      break
    }
